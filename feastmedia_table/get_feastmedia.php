...
$query = $conn->query("SELECT feast_media_event_id, user_id, event_name, ticket_type, event_type, ticket_cost, no_of_tickets_bought, total_cost FROM feastmedia {$search_where} ORDER BY {$columns_arr[$order[0]['column']]} {$order[0]['dir']} limit {$length} offset {$start}");
...
$recordsFilterCount = $conn->query("SELECT feast_media_event_id, user_id, event_name, ticket_type, event_type, ticket_cost, no_of_tickets_bought, total_cost FROM feastmedia {$search_where} ")->num_rows;
