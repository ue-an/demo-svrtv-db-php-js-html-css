...
$totalCount = $conn->query("SELECT * FROM `feastmedia_records` ")->num_rows;
$search_where = "";
if(!empty($search)){
    $search_where = " where ";
...
$query = $conn->query("SELECT feast_media_event_id, attendees.first_name, attendees.last_name, event_name, ticket_type, event_type, event_date, ticket_cost FROM feastmedia_records INNER JOIN attendees ON feastmedia_records.user_id = attendees.user_id {$search_where} ORDER BY {$columns_arr[$order[0]['column']]} {$order[0]['dir']} limit {$length} offset {$start}");
...
$recordsFilterCount = $conn->query("SELECT feast_media_event_id, coalesce(attendees.first_name,'')+' '+ coalesce(attendees.last_name,''), event_name, ticket_type, event_type, event_date, ticket_cost FROM feastmedia_records INNER JOIN attendees ON feastmedia_records.user_id = attendees.user_id {$search_where} ")->num_rows;
